...
export default async function WinningPage({ params }: PageProps<"/app/winnings/[id]">) {
  ...
  const proofUrl =
    record.proofPath && !canSubmitProof(record) ? await service.proofUrl(record.proofPath) : null;
    ...
    <div className="flex flex-col gap-8">
      ...
      <Card className="flex flex-col gap-6">
        <div className="grid gap-6 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-2">
            <p className="text-sm font-medium uppercase tracking-[0.06em] text-ink-2">
              {formatMonth(record.drawMonth)} draw
            </p>
        ...
        {record.review === "rejected" && record.reviewNote && (
          <Banner tone="danger">
            ...
            {canSubmitProof(record)
              ? "You can upload one more screenshot."
              : "This was the second review, so the claim is closed."}
          </Banner>
        )}
        {record.payout === "paid" && (
      ...
      <Card className="flex flex-col gap-4">
        ...
        <p className="text-sm text-ink-2">
          A screenshot from the app or club system where these rounds were recorded, showing the
          five scores and their dates. PNG, JPG or WebP, up to 5 MB. Only you and the admin can see
          it.
        </p>
        {canSubmitProof(record) ? (
