...
export function SubscriptionBanner({ subscription }: { subscription: SubscriptionState }) {
  ...
  if (status === "active" && cancelAtPeriodEnd && currentPeriodEnd) {
    return <Banner tone="warn">Active until {formatDay(currentPeriodEnd)} · won&apos;t renew. {link}</Banner>;
  }
  if (status === "past_due") {
    return <Banner tone="danger">Your last payment didn&apos;t go through. Update your card to stay in the draw. {link}</Banner>;
  }
  if (status === "lapsed" || status === "cancelled") {
    return <Banner tone="warn">Your subscription has {status === "lapsed" ? "lapsed" : "ended"}. Your scores are safe — renew to enter the next draw. {link}</Banner>;
  }
  if (status === "none") {
    return <Banner tone="saffron">You&apos;re not subscribed yet. Subscribe to log scores and enter the monthly draw. {link}</Banner>;
  }
  return null;
}
