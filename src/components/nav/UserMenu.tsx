  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div
          role="menu"
          className="absolute right-0 mt-2 w-48 rounded-md border border-line bg-surface p-1 text-sm"
        >
          ...
          <form action={logOut}>
            <button
              type="submit"
              role="menuitem"
              className="w-full rounded px-3 py-2 text-left text-ink-2 hover:bg-surface-2 hover:text-ink"
            >
              Log out
            </button>
          </form>
...
function MenuItem({ href, children }: { href: string; children: string }) {
  return (
    <Link
      href={href}
      role="menuitem"
      className="block rounded px-3 py-2 text-ink-2 hover:bg-surface-2 hover:text-ink"
    >
      {children}
    </Link>
  );
