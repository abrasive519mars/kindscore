...
beforeAll(async () => {
  ...
  [asSubscriber, asNonSubscriber] = await Promise.all([
    clientAs(subscriber),
    clientAs(nonSubscriber),
  ]);
});
...
describe("rolling five (trigger enforce_score_window)", () => {
  it("keeps the five most recent rounds by date played, whatever the insert order", async () => {
    ...
    for (const played_on of dates) {
      const { error } = await asSubscriber
        .from("scores")
        .insert({ user_id: subscriber.id, score: 30, played_on });
      expect(error).toBeNull();
    }
    expect(await keptDates(subscriber.id)).toEqual([
      "2026-09-25",
      "2026-09-20",
      "2026-09-15",
      "2026-09-10",
  ...
  it("evicts the right round when a backdated one lands inside the window", async () => {
    const { error } = await asSubscriber
      .from("scores")
      .insert({ user_id: subscriber.id, score: 31, played_on: "2026-09-12" });
    expect(error).toBeNull();
    expect(await keptDates(subscriber.id)).toEqual([
      "2026-09-25",
      "2026-09-20",
...
describe("constraints", () => {
  it("rejects a second round on the same date", async () => {
    const { error } = await asSubscriber
      .from("scores")
      .insert({ user_id: subscriber.id, score: 40, played_on: "2026-09-25" });
  ...
  it.each([0, 46])("rejects a score of %s", async (score) => {
    const { error } = await asSubscriber
      .from("scores")
      .insert({ user_id: subscriber.id, score, played_on: "2026-08-01" });
...
describe("access gate (§04)", () => {
  it("refuses a score from a member without an active subscription", async () => {
    const { error } = await asNonSubscriber
      .from("scores")
      .insert({ user_id: nonSubscriber.id, score: 30, played_on: "2026-09-01" });
  ...
  it("refuses a score written under another member's id", async () => {
    const { error } = await asSubscriber
      .from("scores")
      .insert({ user_id: nonSubscriber.id, score: 30, played_on: "2026-09-02" });
    expect(error).not.toBeNull();
  });
});
