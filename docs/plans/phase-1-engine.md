...
`buildFrequencyMap(entries): ReadonlyMap<number, number>` — for each entry, count each **distinct** score once (a user with `33, 33, …` adds 1 to 33, not 2).
...
| Function | Contract |
|---|---|
| `countMatches(scores, drawn)` | `|set(scores) ∩ set(drawn)|` |
...
| `scores/dates.test.ts` | IST midnight: `2026-09-12T19:00:00Z` → `"2026-09-13"`; `2026-09-12T18:00:00Z` → `"2026-09-12"`; `2026-02-30` invalid; future detection |
| `scores/window.test.ts` | 6th evicts oldest by `playedOn` not insert order; duplicate date → `ConflictError`; backdated beyond window → `RuleViolationError`; backdated within window accepted and evicts the correct one; 4 entries + new → nothing evicted |
...
2. scores (validate, dates, window)
