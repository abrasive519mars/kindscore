...
**Goal:** a person can sign up (picking a charity and their percentage), log in, log out, and land in a member area that knows *exactly* what they may do; an admin lands in an admin area; everyone else is turned away — all enforced on the server, on every request, per PRD §04. Plus the UI primitives every later screen is built from.
...
### 4.2 `src/proxy.ts`
`export async function proxy(request)`: `updateSession` → if path starts with `/app` or `/admin` and no claims → redirect `/login?next=<path>`. If path is `/login` or `/signup` and there *is* a session → redirect `/app`. `config.matcher` excludes `_next/static`, `_next/image`, `favicon`, `icon.svg`, `public/seed/**`.
...
- **[decision] Signup is one page** (account · charity · percentage). Plan and payment follow *after* the account exists, so an abandoned checkout still leaves a member we can win back — and so the split slider is seen before any money is asked for.
