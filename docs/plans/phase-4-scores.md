...
The engine decides _and explains_ (it names the evicted round so the UI can say "replaced your 19 Aug score"). The trigger guarantees. The service is the only place that knows both exist.
...
| `update(userId, id, input)` | list → must own `id` (404) → if date changed, `findEntryOnDate` on the _others_ (409) → repo.update |
...
- `ScoreForm`: number input (`inputmode=numeric`, 1–45, stepper buttons ±1 for thumb use), date input (default today in IST, max today), `useActionState(addScore)`. On success: inline confirmation _"Saved. Replaced your 19 Aug round (31)."_ when something was evicted, else _"Saved."_. On `CONFLICT`: the message plus an **Edit that round** link that focuses the existing entry. On `RULE_VIOLATION`: the message.
