...
Rules:
- The system keeps only the user's **latest 5 scores** — latest by the *date played*, not by when it was typed in. Entering a 6th automatically drops the oldest-dated one.
...
"Jackpot" is just the name for the 5-match prize — the biggest and hardest to win. Most months **nobody** matches all 5. When that happens, the PRD says the jackpot money **rolls over**: it is carried forward and added to *next* month's 5-match prize.
...
- If any eligible user's scores change *between* simulate and publish, the draft is marked **stale** and Publish is disabled until the admin re-simulates. The admin never publishes a result that no longer matches what members have.
